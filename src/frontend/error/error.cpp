...
ErrorManager::ErrorManager() : _errors(ParseError::Vec()), errorCount(0), warningCount(0) {}
...
void ErrorManager::plainError(ParseError::Type type, string title,
    const string &file, Marking::Vec marks, bool aggressive) {
        return plainError(type, title, Metadata(file), marks, aggressive);
}
...
void ErrorManager::plainWarning(ParseError::Type type, string title,
    const string &file, Marking::Vec marks, bool aggressive) {
        return plainWarning(type, title, Metadata(file), marks, aggressive);
}
